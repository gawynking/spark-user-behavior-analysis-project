package chavin.king.sparkproject.spark.session;
...
import chavin.king.sparkproject.conf.ConfigurationManager;
import chavin.king.sparkproject.constant.Constants;
...
import chavin.king.sparkproject.domain.SessionDetail;
import chavin.king.sparkproject.domain.SessionRandomExtract;
import chavin.king.sparkproject.domain.Task;
import chavin.king.sparkproject.domain.Top10Category;
import chavin.king.sparkproject.domain.Top10Session;
import chavin.king.sparkproject.test.MockData;
import chavin.king.sparkproject.utils.DateUtils;
import chavin.king.sparkproject.utils.NumberUtils;
import chavin.king.sparkproject.utils.ParamUtils;
import chavin.king.sparkproject.utils.SparkUtils;
import chavin.king.sparkproject.utils.StringUtils;
import chavin.king.sparkproject.utils.ValidUtils;
...
public class UserVisitSessionAnalyzeSpark {
	...
	public static void main(String[] args) {
		...
		/**
		 * actionRDD，就是一个公共RDD 第一，要用ationRDD，获取到一个公共的sessionid为key的PairRDD
		 ...
		 * 
		 * 从Accumulator中，获取数据，插入数据库的时候，一定要，一定要，是在有某一个action操作以后 再进行。。。
		 * 
		 * 如果没有action的话，那么整个程序根本不会运行。。。
		 * 
		 * 是不是在calculateAndPersisitAggrStat方法之后，运行一个action操作，比如count、take 不对！！！
		 * 
		...
		/**
		 * 特别说明 我们知道，要将上一个功能的session聚合统计数据获取到，就必须是在一个action操作触发job之后
		 ...
		 * 
		 * 如果不进行重构，直接来实现，思路： 1、actionRDD，映射成<sessionid,Row>的格式
		 ...
		 * 
		 * 开发Spark大型复杂项目的一些经验准则： 1、尽量少生成RDD
		 * 2、尽量少对RDD进行算子操作，如果有可能，尽量在一个算子里面，实现多个需要做的功能
		 * 3、尽量少对RDD进行shuffle算子操作，比如groupByKey、reduceByKey、sortByKey（map、
		 * mapToPair） shuffle操作，会导致大量的磁盘读写，严重降低性能
		 * 有shuffle的算子，和没有shuffle的算子，甚至性能，会达到几十分钟，甚至数个小时的差别
		 * 有shfufle的算子，很容易导致数据倾斜，一旦数据倾斜，简直就是性能杀手（完整的解决方案） 4、无论做什么功能，性能第一
		 ...
		 * 代码coding最重要，其实项目，我觉得，最重要的，除了技术本身和项目经验以外；非常重要的一点，就是 积累了，处理各种问题的经验
		 * 
		 */
	...
	private static JavaRDD<Row> getActionRDDByDateRange(SQLContext sqlContext, JSONObject taskParam) {
		...
		/**
		 * 这里就很有可能发生上面说的问题 比如说，Spark SQl默认就给第一个stage设置了20个task，但是根据你的数据量以及算法的复杂度
	...
	private static JavaPairRDD<String, String> aggregateBySession(JavaSparkContext sc, SQLContext sqlContext,
		...
		new PairFunction<Tuple2<String, Iterable<Row>>, Long, String>() {
			...
			public Tuple2<Long, String> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
				...
				String partAggrInfo = Constants.FIELD_SESSION_ID + "=" + sessionid + "|"
						...
						+ stepLength + "|" + Constants.FIELD_START_TIME + "=" + DateUtils.formatTime(startTime);
		...
		// final Broadcast<List<Tuple2<Long, Row>>> userInfosBroadcast =
		// sc.broadcast(userInfos);
		//
		// JavaPairRDD<String, String> sessionid2FullAggrInfoRDD =
		// userid2PartAggrInfoRDD.mapToPair(
		//
		...
		// });
		//
		// JavaPairRDD<String, Row> skewedUserid2infoRDD =
		// userid2InfoRDD.filter(
		//
		...
		//
		// new PairFunction<Tuple2<String,Tuple2<String,Row>>, Long,
		// Tuple2<String, Row>>() {
		//
		// private static final long serialVersionUID = 1L;
		//
		...
		// });
		//
		// JavaPairRDD<String, String> mappedRDD =
		// userid2PartAggrInfoRDD.mapToPair(
		//
		// new PairFunction<Tuple2<Long,String>, String, String>() {
		//
	...
	private static void randomExtractSession(JavaSparkContext sc, final long taskid,
		...
		/**
		 * 每天每小时的session数量的计算 是有可能出现数据倾斜的吧，这个是没有疑问的
		 * 比如说大部分小时，一般访问量也就10万；但是，中午12点的时候，高峰期，一个小时1000万 这个时候，就会发生数据倾斜
		 * 
		 ...
		 * 之前是直接在算子里面使用了这个map，那么根据我们刚才讲的这个原理，每个task都会拷贝一份map副本 还是比较消耗内存和网络传输性能的
		 * 
		 * 将map做成广播变量
		 * 
		 */
		...
		/**
		 * 广播变量，很简单 其实就是SparkContext的broadcast()方法，传入你要广播的变量，即可
		 */
		...
		// extractSessionDetailRDD.foreach(new
		// VoidFunction<Tuple2<String,Tuple2<String,Row>>>() {
		//
		// private static final long serialVersionUID = 1L;
		//
		// @Override
		// public void call(Tuple2<String, Tuple2<String, Row>> tuple) throws
		// Exception {
		// Row row = tuple._2._2;
		//
		...
		//
		// ISessionDetailDAO sessionDetailDAO =
		// DAOFactory.getSessionDetailDAO();
	...
	private static List<Tuple2<CategorySortKey, String>> getTop10Category(long taskid,
		...
		/**
		 * 必须要进行去重 如果不去重的话，会出现重复的categoryid，排序会对重复的categoryid已经countInfo进行排序
		 * 最后很可能会拿到重复的数据
		 */
	...
	private static JavaPairRDD<Long, Long> getClickCategoryId2CountRDD(JavaPairRDD<String, Row> sessionid2detailRDD) {
		 ...
		 * 说明一下：
		 * 
		 * 这儿，是对完整的数据进行了filter过滤，过滤出来点击行为的数据 点击行为的数据其实只占总数据的一小部分
		 * 所以过滤以后的RDD，每个partition的数据量，很有可能跟我们之前说的一样，会很不均匀 而且数据量肯定会变少很多
		 * 
		 ...
		 * local模式自己本身就是进程内模拟的集群来执行，本身性能就很高 而且对并行度、partition数量都有一定的内部的优化
		 * 
		 * 这里我们再自己去设置，就有点画蛇添足
		 * 
		...
		// mappedClickCategoryIdRDD.reduceByKey(
		//
		// new Function2<Long, Long, Long>() {
		//
		// private static final long serialVersionUID = 1L;
	...
	private static JavaPairRDD<Long, String> joinCategoryAndData(JavaPairRDD<Long, Long> categoryidRDD,
		...
		new PairFunction<Tuple2<Long, Tuple2<Long, Optional<Long>>>, Long, String>() {
			...
			public Tuple2<Long, String> call(Tuple2<Long, Tuple2<Long, Optional<Long>>> tuple) throws Exception {
				...
				String value = Constants.FIELD_CATEGORY_ID + "=" + categoryid + "|" + Constants.FIELD_CLICK_COUNT + "="
						+ clickCount;
